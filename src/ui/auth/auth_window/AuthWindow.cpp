...
/* --------------------  Setters  -----------------  */
void AuthWindow::setRightWidget(QWidget *widget) {
    if (!widget || !right)
        return;
...
void AuthWindow::setDarkMode(bool isDarkMode) {
    // Calling Base class member function of theme
    SubWindow::setDarkMode(isDarkMode);

    // Theme Mode Button
    themeMode->setDarkMode(isDarkMode);
    isDarkMode ? themeMode->setIconPaths(LightModeIcon, LightModeIcon) : themeMode->setIconPaths(DarkModeIcon, DarkModeIcon);
...
BulletPoint::BulletPoint(const QString &text, const QString &iconPath, QWidget *parent) : QLabel(parent), text(text) {
   ...
   // Icon 
   icon = IconManager::renderSvg(iconPath, QSize(22, 22));
