...
View::View(QWidget *parent) : QWidget(parent) {
    ...
    // ===== TEST: Auto-remove items to test empty state =====
QTimer *testTimer = new QTimer(this);
testTimer->setInterval(1000); // 1 second

connect(testTimer, &QTimer::timeout, this, [this, testTimer]() {
    int count = _model.rowCount();
    if (count > 0) {
        _model.removeRow(count - 1); // remove last item
    } else {
        testTimer->stop();
...
void View::updateEmptyStatePosition() {
    if (empty_state) {
        ...
        int y = ((height() - empty_state->height()) / 2) + 30;
        empty_state->raise();
