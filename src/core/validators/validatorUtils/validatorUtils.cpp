...
ValidatorUtils::ValidatorUtils(QObject *parent, const QString &alias) : QObject(parent), alias(alias)
{
    ...
    connect(this, &ValidatorUtils::networkError, [this, alias](QNetworkReply::NetworkError err) {
        qDebug() << "Downloading " << alias << " blacklist failed: " << err << "\n";
    });
}
...
void ValidatorUtils::onFinished(QNetworkReply *reply)
{
    ...
    if (reply->error() != QNetworkReply::NoError) {
        ...
        QTimer::singleShot(10000, this, [this, url](){  // Reattempt after 10 seconds to download again if failed
            downloadList(url);
        }); 
